	...
	public function commonProcess($modelName = null, array $options = array()) {
		...
		if (!empty($this->controller->request->data)) {
			...
			if ($valid) {
				...
				$searchParams = $this->normalize($searchParams); //clear empty multycheck
 ...
 */
	...
	public function normalize($data = []){
        ...
        $presetVars = [];
        if(! empty($this->controller->presetVars)){
            foreach ($this->controller->presetVars as $key => $presetVar){
                if(! empty($presetVar['field'])){
                    $presetVars[$presetVar['field']] = $presetVar;
                }
            }
        }
		array_walk($data, function(&$values, $field) use ($presetVars){
			$values = PrgComponent::_normalize($values, $field, $presetVars);
		});
		return $data;
	}

	protected static function _normalize($data = null, $field = null, array $presetVars = []){
		if(is_array($data)){
			$_data = [];
			foreach($data as $key => $value){
			    if(empty($presetVars[$field]['allowEmpty'])){
                    if(! empty($value)){
                        $_data[$key] = $value;
                    }
                }else{
