...
class SearchableBehavior extends ModelBehavior {
    ...
    public function setup(Model $Model, $config = array()) {
        ...
        if (empty($Model->filterArgs) && !empty($this->settings[$Model->alias]['filterArgs']))
        {
            $Model->filterArgs = $this->settings[$Model->alias]['filterArgs'];
        }
    }
    ...
    public function gt($Model, $data, $field)
    {
        return [$field['field'].' >' => $data[$field['name']]];
    }

    public function gte($Model, $data, $field)
    {
        return [$field['field'].' >=' => $data[$field['name']]];
    }

    public function lt($Model, $data, $field)
    {
        return [$field['field'].' <' => $data[$field['name']]];
    }

    public function lte($Model, $data, $field)
    {
        return [$field['field'].' <=' => $data[$field['name']]];
    }

    /***
     * Doesn't affects results. It may be used to additional processing in controller
     * @param $Model
     * @param $data
     * @param $field
     * @return array
     */
    public function noAction($Model, $data, $field)
    {
        return [];
    }
    ...
    protected function _addCondSubquery(Model $Model, &$conditions, $data, $field) {
        ...
        if ((method_exists($Model, $field['method']) || $this->_checkBehaviorMethods($Model, $field['method'])) && (!empty($field['allowEmpty']) || !empty($data[$field['name']]) || (isset($data[$field['name']]) && (string)$data[$field['name']] !== ''))) {
            ...
            if (!empty($subquery)) {
                //https://pm.is2b.ru/project_tasks/view/39000
                if(! isset($this->subqueryIndex[$Model->alias])){
                    $this->subqueryIndex[$Model->alias] = [];
                }
                $iS = md5($subquery);
                if(empty($this->subqueryIndex[$Model->alias][$iS])){
                    $conditions[] = $Model->getDataSource()->expression("$fieldName in ($subquery)");
                    $this->subqueryIndex[$Model->alias][$iS] = 1;
                }
            }
        }
        return $conditions;
